...
const phishingPatterns = [
  ...
  /\bsuspicious (?:caller|number|contact)\b/i,
  /\bcaller .{0,50}(?:call|contact).{0,50}(?:number|account|fix)\b/i,
  /\bcall .{0,40}(?:number|caller).{0,40}(?:fix|account)\b/i,
  /\bcontact .{0,40}(?:number|caller).{0,40}(?:called|contacted)\b/i,
  /\bnumber that called me\b/i,
...
const wrongTransferPatterns = [
  ...
  /\bwrongly sent\b/i,
  /\bsent .{0,40}(?:brother|recipient|person).{0,40}(?:didn'?t|get|receive)/i,
  /\b(?:brother|recipient|person).{0,40}(?:didn'?t|get|receive)/i,
  /ভুল নম্বর/i,
  /ভুল নাম্বার/i,
  /ভুলে পাঠিয়েছি/i,
  /ভুল একাউন্ট/i
